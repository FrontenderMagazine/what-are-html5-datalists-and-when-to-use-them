...
Всем интернет-пользователям знакома такая функция как автозавершение. Когда вы 
используете поиск, поисковик предлагает разные варианты. Когда вы составляете 
имейл-сообщение, ваш имейл-клиент предлагает список получателей. Однако для 
добавления этой функции веб-разработчикам приходится использовать значительное 
количество JavaScript. Один из новых элементов HTML5, `<datalist>`, позволяет 
добавлять функцию автозавершения без использования JavaScript. 
...
Чтобы продемонстрировать как работает список вариантов, для начала добавим в код 
...
Обратите что атрибут `list` элемента `input` и атрибут `id` элемента `<datalist>` 
содержат одинаковое значение «team_list». Это их связывает. 
...
Рисунок 1. Вид списка вариантов (верх слева: Internet Explorer 10; верх справа: 
Firefox 18; низ слева: Chrome 24; низ справа: Opera 12)

Примечание: в Internet Explorer 10 и Opera пользователю не нужно вводить букву, 
список вариантов отображается сразу, в отличии от Firefox и Chrome.
...
функция автозаполнения введена уже давно. Во всех браузерах есть механизм 
...
Так в чём же разница между атрибутом `autocomplete` и списком вариантов 
...
Хотя автозаполнение обычно ассоциируется с вводом текста, списки вариантов можно 
также использовать и для некоторых новых типов `input`, добавленных в HTML5. 
...
Как видите, в обеих браузерах для каждого `option` нашего списка `datalist` 
...
К сожалению, в данный момент формы с ползунком, созданные с помощью `input` с 
типом `range` поддерживаются только в Internet Explorer и Chrome. На рисунке 5 
показана поддержка использования `<datalist>` для распространённых типов `input` 
...
Так как у списков `<datalist>` нет встроенного механизма, требующего чтобы 
пользователь выбрал один из предложенных вариантов, `<datalist>` хорошо подходят 
для полей ввода которые принимают любое значение. Как в примере с любимой 
командой, приведенном выше. Хоть список с вариантами названий команд и был 
предложен, пользователь мог ввести любое значение. 

С другой стороны, пример с американскими штатами не подходит так как количество 
валидных значений, которые пользователь может ввести, строго ограничен. В таком 
случае лучше использовать элемент `select`, тогда выбор из списка будет 
обязательным.

Исключением будут случаи когда количество валидных значений огромно. Например, 
взгляните на распространённый выпадающий список для выбора страны, изображённый 
на рисунке 6.
...
Этот список неудобен для пользователей, так как им приходится пролистать сотни 
вариантов чтобы найти нужный. В этом случае функция автозаполнения более удобна, 
так как пользователь получает возможность быстро отфильтровать список введя 
одну-две буквы. 
...
Рисунок 7. Использование списка вариантов `<datalist>` для создания формы ввода 
страны
...
сделает. Например, в следующем коде такая валидация добавлена с помощью [API 
проверки ограничений валидации][1]: 
...
валидация на стороне сервера так же необходима. Пользователи, использующие 
...
интерфейс непригодным к эксплуатации в браузерах, не поддерживающих его. 
Пользователь получает указание выбрать валидное значение из предложенных 
вариантов, которые не видит если его браузер не поддерживает `datalist`. Так что 
если вы планируете использовать этот подход, необходимо продумать интерфейс, 
который будет работать во всех браузерах. Этого можно достичь выполняя проверку 
на поддержку `datalist` браузером и используя подходящий полифил.

## Браузеры, не поддерживающие `datalist`

Во время написания статьи списки вариантов для текстовых полей поддерживаются в 
...
Одно из решений, [популяризованное Джереми Кейтом (Jeremy Keith)][2], состоит в 
...
пример с списком стран чтобы добавить резервное решение используя `<select>`
...
автозавершения как `datalist`. Если для вашего списка вариантов необходимо 
автозавершение, воспользуйтесь вторым резервным решением с использованием 
полифила.

Сначала нужно определить поддерживает ли браузер `datalist`. Такую проверку 
...
есть [несколько полифилов][5], но я отдаю предпочтение виджету автозавершения 
...
используется полифил автозавершения на основе jQuery.

![Рисунок10][Список вариантов названий стран с использованием виджета автозавершения на основе jQuery в качестве полифила]

Рисунок 10. Список вариантов названий стран с использованием виджета 
автозавершения на основе jQuery в качестве полифила (Safari)

Вы возможно заметили что по умолчанию jQuery-виджет автозавершения предлагает 
...
введёнными пользователем. В отличие от `datalist`, в виджете автозавершения 
можно настроить принцип выдачи вариантов на своё усмотренение. 

Следующий пример демонстрирует как можно настроить функцию автозавершения, 
которая предлагает варианты, в которых первые символы совпадают с введёнными: 
    ...
    $('#autocomplete').autocomplete({
      // в req будет помещён объект с свойством, которое содержит значение, 
...
Первое: Internet Explorer и старше требует чтобы все элементы `option` были 
...
Теперь Internet Explorer 9 и старше будет понимать элементы `option` так, как 
...
полей, ему не хватает настроек, которые нужны для многих современных 
...
решение для автозавершения. Виджет автозавершения jQuery даёт все перечисленные 
...
виджете автозавершения можно найти в [демо][7] и документации по API (Виджет 
автозавершения работает только для текстовых полей ввода, его нельзя применить 
...
резервное решение в виде полифила для браузеров, которые не поддерживают 
`datalist`.

Список вариантов `datalist` является отличным инструментом для вывода вариантов 
автозавершения, однако его функциональность ограничена. Если вам нужно более 
сложное решение, вам следует рассмотреть возможность использования виджета 
автозавершения на jQuery. 
...
[Список вариантов названий стран с использованием виджета автозавершения на основе jQuery в качестве полифила]: img/figure10-ru.png